...
## Background — BE contract (đã verify từ `BE/openapi.json`, refresh 2026-05-28)
...
| `/backtest/{backtest_id}/candles` 🆕 | GET    | path `backtest_id:int`               | —                 | `200` → `BacktestCandlesResponse` (xem Task 5) |
...
// BacktestRequest (POST body)
{
  ...
  timerange?: string | null;     // BE description: "Timerange in format YYYYMMDD-YYYYMMDD" — Gap 1 resolved
...
// BacktestHistoryItem (GET result)
{
  ...
  total_profit?: number | null;    // ✅ Gap 2 resolved (2026-05-28): absolute USDT/USDC
                                   //    (= results.strategy_comparison[0].profit_total_abs;
                                   //    sample backtest_200.json: -36.5898 ≈ -36.58987438)
  win_rate?: number | null;        // percentage 0-100 (sample: 44.23 = 44.23%) — formatWinRate vẫn dual-unit
  started_at: string;
  completed_at?: string | null;    // != null ⇒ đã xong
  results?: object | null;         // additionalProperties:true trong spec,
                                   // NHƯNG sample đã xác nhận shape (xem Task 3 extractMetrics):
                                   //   { strategy: { [name]: RawFreqtradeReport },
                                   //     strategy_comparison: Array<BacktestComparisonItem> }
}

// BacktestHistoryList
{ items: BacktestHistoryItem[]; total: number; }

// BacktestCandlesResponse (NEW — Tuấn thêm cho equity curve)
{ backtest_id: number; pair: string; timeframe: string;
  range_start: number; range_end: number;       // ms timestamps
  candles: unknown[]; }                          // OHLCV array; shape FE-defined khi build equity curve
```

**`BacktestComparisonItem` shape** (FE-defined local type, suy từ sample `BE/backtest_200.json`):

```ts
interface BacktestComparisonItem {
  key: string; // strategy name (e.g. "Gamma")
  trades: number;
  profit_mean: number; // ratio per-trade
  profit_mean_pct: number; // percentage per-trade
  profit_total: number; // ratio total
  profit_total_pct: number; // percentage total
  profit_total_abs: number; // absolute USDT total ← match BacktestHistoryItem.total_profit
  duration_avg: string; // "Hh MMm SSs"
  wins: number;
  draws: number;
  losses: number;
  winrate: number; // 0-1 ratio (vs top-level win_rate which is 0-100)
  cagr: number;
  expectancy: number;
  ...
  max_drawdown_abs: string; // ⚠️ string trong sample (e.g. "96.933"), không phải number — parseFloat khi dùng
}
```

**Reference đọc thêm:** `BE/API_SPEC.md`, `src/features/bot-builder/bot-strategy.api.ts` (pattern api), `src/features/export-import/ExportDialog.tsx` (pattern Radix dialog + toast + error), `src/features/bot-monitoring/bot.api.ts` (pattern http + types). Sample backtest response: `BE/backtest_200.json` (local-only, dùng kiểm tra shape thật).
...
| `src/features/backtest/backtest-helpers.ts`      | Pure: `presetToTimerange`, `isBacktestTerminal`, `extractMetrics`, `formatWinRate`, `formatTotalProfit` |
...
describe('extractMetrics', () => {
  ...
  const base: BacktestHistoryItem = {
    ...
    results: {
      strategy: { Gamma: {} }, // raw Freqtrade report — not used by extractMetrics
      strategy_comparison: [
        {
          key: 'Gamma',
          trades: 104,
          ...
          max_drawdown_account: 0.0953, // 0-1 ratio → 9.53%
          max_drawdown_abs: '96.933', // string in sample
        },
      ],
    },
  };
