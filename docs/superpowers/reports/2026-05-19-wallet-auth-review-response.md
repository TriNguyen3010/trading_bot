...
| ✅ **Resolved 2026-05-19 bởi BE** | 1 | C3 (BE Tuấn: deactivated → 403 + message) |
...
**Verdict:** ✅ **RESOLVED 2026-05-19 — BE Tuấn confirm scenario (a): middleware block deactivated user → trả 403 + message.**

**BE answer (2026-05-19):**
> "deactivate thì bên e sẽ trả 403 kèm message nha a"

**Hệ quả:**
- FE 403 handling đã có sẵn (toast với BE message + giữ cred + không redirect) → cover được deactivated case.
- `is_active` field trong response `/user/status` về lý thuyết luôn `true` (BE middleware block trước khi tới handler).
- FE **vẫn giữ defensive check** `if (!user.is_active)` trong `connect()` flow như belt-and-suspenders cho future BE policy change (xem spec §3.2 step 6 + §14.4).

**UX trade-off documented** (spec §14.4):
- Nếu user đang ở `/builder` rồi mới bị deactivate → mỗi API call → 403 toast → effectively stuck.
- Admin reactivation là recourse duy nhất.
- Acceptable cho MVP — edge case admin action không thường xuyên.

**Fix đã apply (xem commit sau commit `717e319`):**
- Spec §3.2 step 6: thêm defensive `is_active` check
- Spec §4.4: bảng 403 thêm row "User deactivated"
- Spec §10 Q10: log BE confirmation
- Spec §14.4 mới: document UX limitation
- Plan http.ts: 403 toast pass-through BE `{detail}` message
- Plan http.test.ts: 3 test cases cho 403 (JSON, plain text, empty)
- Plan wallet.store.ts `connect()`: defensive `is_active` check
- Plan wallet.store.test.ts: thêm test "shows error when user is inactive"
...
| # | Mô tả | Action |
|---|---|---|
| **C3** | `is_active=false` semantics | ✅ **DONE** — BE Tuấn 2026-05-19: deactivated → 403 + message. FE thêm defensive check + §14.4 UX note. |
