...
| `src/features/bot-monitoring/BotMonitoringPage.tsx`            | Bỏ wrapper `flex min-h-screen flex-col` + halo. Early-return loading đổi thành chiếm vùng main (bỏ `h-screen w-screen bg-black`). Giữ DotGrid + main + dialogs.                                                                                  | **Sửa**     |
