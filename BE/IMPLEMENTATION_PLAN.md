...
>
> **Bối cảnh codebase**: Project đã có `zod`, `@hookform/resolvers`, `vitest`, và 3 file Zod schemas hiện tại (`src/schemas/{bot,strategy,bundle}.schema.ts`) **đang mirror format LEGACY**. Plan này KHÔNG build từ đầu — mà refactor những thứ đã có cho khớp `BE/openapi.json`.
>
...
```json
{
  "gen:api": "openapi-typescript BE/openapi.json -o src/types/api.d.ts",
  "gen:api:watch": "openapi-typescript BE/openapi.json -o src/types/api.d.ts --watch"
}
```
...
**2.2.1. Tạo `BE/payload_bot_strategy_create.json`**

Lấy nội dung từ `BE/payload_create_bot.json` + `BE/payload_create_strategy.json`, áp dụng migration mapping (theo `API_SPEC.md` mục 8):
...
**2.2.2. Tạo `BE/payload_bot_strategy_update.json`**
...
```bash
node scripts/validate-sample.mjs BE/payload_bot_strategy_create.json
```
...
| File                                  | Thao tác                                                                |
| ------------------------------------- | ----------------------------------------------------------------------- |
| `BE/payload_bot_strategy_create.json` | **Tạo mới**                                                             |
| `BE/payload_bot_strategy_update.json` | **Tạo mới**                                                             |
| `scripts/validate-sample.mjs`         | **Tạo mới** (tạm, có thể xoá sau B3)                                    |
| `BE/payload_create_bot.json`          | **Giữ lại** — đổi tên thành `payload_create_bot.legacy.json` để rõ ràng |
| `BE/payload_create_strategy.json`     | **Giữ lại** — đổi tên thành `payload_create_strategy.legacy.json`       |
...
Trở thành schema cho **bot runtime fields** (subset của UnifiedBotStrategyCreate, không có configurations/strategy\_\*):
...
strategyConfigurationsSchema.superRefine((data, ctx) => {
  ...
  const declaredIndicators = new Set([
    ...data.signals.indicators.map(
      (i) => `${i.name}-${i.parameters?.timeperiod ?? ''}`,
    ),
  ...
  if (minutes.some((m, i) => i > 0 && m <= minutes[i - 1])) {
    ctx.addIssue({
      code: 'custom',
      path: ['roi_steps'],
      message: 'roi_steps phải sort theo minutes tăng dần',
    });
  }
  ...
  if (hasShortConditions && !data.can_short) {
    ctx.addIssue({
      code: 'custom',
      path: ['can_short'],
      message: 'Phải bật can_short khi có short conditions',
    });
  }
  ...
  if (data.custom_exit.partial_enabled && !data.position_adjustment_enable) {
    ctx.addIssue({
      ...
      message: 'Phải bật khi custom_exit.partial_enabled=true',
    });
  }

  // (5) custom_exit.partial_levels: tổng amount phải <= 100
  const totalAmount = data.custom_exit.partial_levels.reduce(
    (s, lv) => s + lv.amount,
    0,
  );
  if (totalAmount > 100) {
    ctx.addIssue({
      ...
      message: `Tổng amount = ${totalAmount}%, phải <= 100%`,
    });
  }
});
```
...
export const unifiedBotStrategyUpdateSchema = z
  .object({
    // tất cả optional
  })
  .partial();
...
- **Good fixtures**:
  - parse `BE/payload_bot_strategy_create.json` (từ Bước 2) → expect pass.
  - parse `request` của `BE/user_2_bot_strategy_create_POST_20260428_042734.json` (log production status 201) → expect pass. Nếu fail → schema FE đang strict hơn BE thực tế (không tốt).
...
- [ ] `BE/payload_bot_strategy_create.json` + `payload_bot_strategy_update.json` tạo xong, validate pass.
