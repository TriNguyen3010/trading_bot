...
describe('Slider', () => {
  it('renders the input with the given value', () => {
    render(<Slider value={25} onValueChange={() => {}} min={1} max={50} ariaLabel="Leverage" />);
  ...
  it('fires onValueChange with a number when user moves the slider', () => {
    ...
    render(<Slider value={1} onValueChange={onValueChange} min={1} max={50} ariaLabel="Leverage" />);
    const input = screen.getByRole('slider', { name: 'Leverage' }) as HTMLInputElement;
  ...
  it('displays the value with the suffix beside the slider', () => {
    render(<Slider value={10} onValueChange={() => {}} min={1} max={50} suffix="x" ariaLabel="Leverage" />);
